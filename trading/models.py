...
class Trade(models.Model):
    ...
    # Standalone now; later we'll change this to FK to marketdata.Asset
    ...
    def rr(self):
        if not self.stop_loss or not self.take_profit:
            return None
        entry, sl, tp = float(self.entry_price), float(self.stop_loss), float(self.take_profit)
        if self.side == 'LONG':
            risk = entry - sl
            reward = tp - entry
        else:
    ...
    def realized_pnl(self):
        if self.status != 'CLOSED' or not self.exit_price:
            return None
        entry, exit_, qty = float(self.entry_price), float(self.exit_price), float(self.quantity)
    ...
    @cached_property
    def asset_obj(self):
        """Return matching marketdata.Asset by symbol (case-insensitive), or None."""
        # Local import avoids potential circular import issues at import time.
        from marketdata.models import Asset
        try:
            return Asset.objects.get(symbol__iexact=self.symbol)
